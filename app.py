...
def fixtures_redirect():
    return redirect("/premier-league?tab=fixtures", code=301)
...
@app.route('/epl_table')
def epl_table():
    return redirect("/premier-league?tab=table", code=301)
...
@app.route("/teams")
def teams_landing():
    return redirect("/premier-league?tab=teams", code=301)
...
def premier_league():
    ...
    gameweeks = sorted(fixtures_df["round_number"].dropna().unique().tolist())

    if GW_OVERRIDE is not None:
        current_gw = GW_OVERRIDE
    else:
        upcoming_rounds = fixtures_df[fixtures_df["isResult"] == False]["round_number"]
        current_gw = int(upcoming_rounds.min()) if not upcoming_rounds.empty else int(max(gameweeks))

    gw = int(request.args.get("gw", current_gw))

    gw_fixtures = fixtures_df[fixtures_df["round_number"] == gw].copy()
            ...
            try:
                ...
                for _, fp in fpl_df.iterrows():
                    ...
                    if t_r > 0:
                        ...
                        p_r = player_recent_xg.get((pname, fteam)) or \
                            player_recent_xg.get((web, fteam)) or \
                ...
                for _, fp in fpl_df.iterrows():
                    ...
                    if not match:
                        web = _norm(fp["web_name"])
                        remapped = FPL_TO_UNDERSTAT.get(web, web)
                        match = player_xg_map.get(remapped) or next(
        ...
        for team in teams:
            ...
            for gw in range(current_gw, current_gw + 5):
                ...
                if gw_fix.empty:
                    ...
                    is_dgw   = len(parts) > 1
                    top_diff = "easy" if all(p["difficulty"] == "easy" for p in parts) else "hard" if all(p["difficulty"] == "hard" for p in parts) else "medium"
...
#def f1_hub():
    try:
        ...
        return render_template("f1_hub.html",
            schedule=data["schedule"],
            next_race_predictions=next_pred,
        ...
        print(f"❌ F1 hub error: {e}")
        import traceback; traceback.print_exc()
        return render_template("f1_hub.html",
            schedule=[], driver_standings=[], constructor_standings=[],
            last_race=None, next_race=None, dpr={}, cpi={},
            next_race_predictions=None,
            today=datetime.now().strftime('%Y-%m-%d'),
            last_updated=get_last_updated_time()
        )
...
#def f1_race(year, round_num):
    try:
        ...
        return render_template("f1_race.html",
            ...
            teammate_deltas=data["teammate_deltas"],
            stint_efficiency=data["stint_efficiency"],
            has_lap_data=data["has_lap_data"],
            last_updated=get_last_updated_time()
        )
    except Exception as e:
        print(f"❌ F1 race report error: {e}")
        import traceback; traceback.print_exc()
        return render_template("f1_race.html",
            year=year, round=round_num, meta={},
            results=[], qualifying=[], stints=[], weather=[],
            teammate_deltas=[], stint_efficiency=[],
            has_lap_data=False,
            last_updated=get_last_updated_time()
        )
...
#def f1_predictions(year, round_num):
    try:
        ...
        return render_template("f1_predictions.html",
            ...
            has_quali=data["has_quali"],
            mode=data["mode"],
            overtaking_index=data["overtaking_index"],
        ...
        print(f"❌ F1 predictions error: {e}")
        import traceback; traceback.print_exc()
        return render_template("f1_predictions.html",
            year=year, round=round_num, race_meta={},
            predictions=[], has_quali=False, mode="pre_quali",
            overtaking_index=0.55, is_completed=False,
            n_simulations=10000, prev_round=None, next_round=None,
            last_updated=get_last_updated_time()
        )
...
#def f1_fantasy():
    try:
        ...
        return render_template("f1_fantasy.html",
            next_race=data["next_race"],
            next_race_xfp=data["next_race_xfp"],
            season_projection=data["season_projection"],
            transfer_targets=data["transfer_targets"],
            races_remaining=data["races_remaining"],
            last_updated=get_last_updated_time()
        )
    except Exception as e:
        print(f"❌ F1 fantasy error: {e}")
        import traceback; traceback.print_exc()
        return render_template("f1_fantasy.html",
            next_race=None, next_race_xfp=[],
            season_projection=[], transfer_targets={},
            races_remaining=0,
            last_updated=get_last_updated_time()
        )
